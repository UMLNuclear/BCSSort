...
public:
  ...
  static TChannel* FindChannel(int);
  ...
  static int ReadDetMapFile(const char* filename="/home/zhu/packages/BCSSort/config/detmap_10.txt",Option_t *opt="replace");
  ...
  void SetSegment(int temp) { segment = temp; }
  ...
  //void SetCrosstalkCoeff(std::vector<double> coeff);
...
public:
  ...
  //static double FixCrosstalk(const std::vector<double>& energy, const std::vector<double>& coeff);
  ...
  //std::vector<double> crosstalk_coeff;
