...
void print(){
  ...
  chan->Add("/home/zhu/packages/BCSSort/CORAnalysis/datafile/event/allimp/event1040*.root");
  ...
  for(x;x<nentries;x++){
    ...
    //if(fevent->LGFSize()!=1 || fevent->LGBSize()!=1) continue;
    double pin1e = fevent->Pin1E();
    double tof   = fevent->I2S();
    int lgfsize = fevent->LGFSize();
    int lgbsize = fevent->LGBSize();
    for(int m=0;m<1;m++){
      if(lblob[m]->IsInside(tof,pin1e)){
        printf("\nEntry \t blob \t HGF.N \t HGB.N \t HGF.C \t HGB.C\n");
