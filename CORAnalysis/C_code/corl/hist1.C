...
void hist1(){
  ...
  for(int runnum = 1040;runnum<1050;runnum++){
    ...
    chan->Add(Form("/home/zhu/packages/BCSSort/CORAnalysis/example/data/correlation1/corlgeo/event%i*.root",runnum));
  ...
  TFile *newf = new TFile("/home/zhu/packages/BCSSort/CORAnalysis/example/hist/cor1_hist.root","recreate");
  h0->Write();
  e0->Write();
  g0->Write();
  for(int i=0;i<6;i++){
