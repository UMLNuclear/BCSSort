...

{
  ...
  TH2D *dt_singles_ne30  = (TH2D *)_file0 ->Get("recae0");
  TH2D *dt_singles_ne31  = (TH2D *)_file0 ->Get("recae1");

  TH2D *cdt0 = (TH2D *)dt_singles_ne30 ->Clone(Form("%s_c",dt_singles_ne30->GetName()) );
  TH2D *cdt1 = (TH2D *)dt_singles_ne31 ->Clone(Form("%s_c",dt_singles_ne31->GetName()));

  int rebin = 10; // bin in 1ms;
  int time = 30;
  ...
  double upper[16]  = {152, 368, 376, 387, 427, 460, 605, 626, 701, 850, 1061, 1123, 1189, 2269, 3182, 3696};
  ...
  gr0  ->SetTitle(Form("30ne_%.1f", energy[j]));
  gr1  ->SetTitle(Form("31ne_%.1f", energy[j]));
