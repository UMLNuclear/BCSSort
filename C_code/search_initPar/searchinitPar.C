...
std::vector<double> searchinitPar(TH1D *hist1, int rebin=1, int binnum=-1, double chi2_min=2.){
  ...
  // Initila Pars Guess
  double p[npar];
  p[0] = 8000./20.*rebin;  // constant bg
  p[1] = 500/20.*rebin;    // exp bg scaling factor
  p[2] = 200;             // exp bg half-life
  p[3] = 1000/20.*rebin;   // 32Na scaling factor in beta- path = 32Mg scaling factor
  p[4] = 11;              // 32Na half-life
  p[5] = 82;              // 32Mg half-life
  p[6] = 27;              // 32Al half-life
  p[7] = 500/20.*rebin;    // 32Na scaling factor in beta-n path = 31Mg scaling factor
  p[8] = 230;             // 31Mg half-life
  ...
  while(chi2>1){
    ...
    if(p[8]>241) { 
      p[8] = 244.1;
      printf("reset p[8] = 236\n"); 
    }
    fx->SetParameter(8,p[8]);  
    fx->SetParLimits(8,229,241); 
    ...
    if((test_chi2 > chi2 && chi2<chi2_min) || (test_chi2<1)){
      printf("chi2 = %f\n", chi2);
      for(int i=0;i<npar;i++){
        printf("p[%i] = %f\n", i,presult[i]);
      }
...
TGraph *FitHL(TH1D *hist1, int rebin=1, int binnum=-1){
  ...
  std::vector<double> p = searchinitPar(hist1, rebin, binnum);
  TH1D *th1 = (TH1D *)hist1->Clone("th1");
  th1->Rebin(rebin);
  double Ymax = th1->GetMaximum();
  int nbins = th1->GetNbinsX();
