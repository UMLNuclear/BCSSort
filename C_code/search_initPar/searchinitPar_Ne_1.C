...
double MYFit(double *x, double *par) {
  ...
  double value = par[0] + MYExp(x,par_bg) + MYExp(x,par1) + Bateman(x,par2,2) + Bateman(x,par3,2) + Bateman(x,par4,3);
...
//TGraph* searchinitPar(TH1D *hist1, int rebin=1, int binnum=-1, double chi2_min=2.){
  ...
  p[2] = 340;               //t1/2_bg;
  p[3] = 5000/20.*rebin;    //A_30Ne
  p[4] = 7.3;               //t1/2_30Ne;
  p[5] = 5000/20.*rebin;      //A_30Na
  p[6] = 40;                //t1/2_30Na
  p[7] = 500/20.*rebin;     //A_29Na;
  p[8] = 44.1;              //t1/2_29Na;
  p[9] = 500/20.*rebin;     //A_30Mg;
  p[10]= 317;               //t1/2_30Mg;
  ...
  fx->FixParameter(6,48);
  fx->FixParameter(8,44.1);
  fx->FixParameter(10,317);
  ...
  while(chi2>1){
    ...
    if(fx->GetParError(4)==0){
      ...
      p[9] = p[9] +10;
      if(p[9]>Ymax) {printf("p[9] = %f\n",p[9]); break;}
      fx->SetParameter(9,p[9]);  

      p[10] = p[10] + 0.5;
      if(p[10]>330){
        p[10] = 310;
        printf("reset p[10] = 310\n");
      }
      fx->SetParameter(10,p[10]);  
    }else{
    ...
    fx->SetParLimits(3,1000,Ymax);     //A_30Ne
    fx->SetParLimits(4,6.3,8.3);       //t1/2_30Ne;
    fx->SetParLimits(5,1,Ymax);        //A_30Na
    fx->SetParLimits(6,46,50);         //t1/2_30Na
    fx->SetParLimits(7,1,Ymax);        //A_29Na
    fx->SetParLimits(8,43,45);         //t1/2_29Na;
    fx->SetParLimits(9,1,Ymax);        //A_30Mg
    fx->SetParLimits(10,310,325);      //t1/2_30Mg;
    ...
    if((test_chi2 > chi2 && test_chi2<chi2_min) || (test_chi2<1) || iteration>100){
      printf("chi2 = %f\n", chi2);
      for(int i=0;i<npar;i++){
        printf("p[%i] = %f\n", i,presult.first[i]);
      }
...
void FitHL(TH1D *hist1, int rebin=1, int binnum=-1, double chi2_min=2){
  ...
  f2->SetParameter(2,p.first[3]);             // actvity
  f2->SetParameter(3,p.first[4]);             // parent t1/2
  ...
  f3->SetParameter(2,p.first[5]);             // acitivity
  f3->SetParameter(3,p.first[4]);             // parent(30Ne) t1/2
  f3->SetParameter(4,p.first[6]);             // daughter(30Na) t1/2
  ...
  f4->SetParameter(2,p.first[7]);             // activity
  f4->SetParameter(3,p.first[4]);             // parent(30Ne) t1/2
  ...
  f5->SetParameter(2,p.first[9]);             // activity
  f5->SetParameter(3,p.first[4]);             // parent(30Ne) t1/2
  f5->SetParameter(4,p.first[6]);             // daughter(29Na) 1/2
  f5->SetParameter(5,p.first[10]);             // grand(30Mg) t1/2
  ...
  gr->GetYaxis()->SetRangeUser(p.first[0]*0.995, Ymax*1.003);
  ...
  //chi2 -> DrawText(0.3, y*.8, Form("X2 = %.2f",p.first.back()));
  //TText *hl = new TText();
  //hl -> SetNDC();
  //hl -> SetTextFont(1);
  //hl -> SetTextColor(1);
  //hl -> SetTextSize(0.05);
  //if(p.second.size()<5){
  //  hl -> DrawText(0.3, y*.75, Form("hl = %.2fms",p.first[4]));
  //}else{
  //  hl -> DrawText(0.3, y*.75, Form("hl = %.2f(%.2f)ms",p.first[4], p.second[4]));
  //} 
  //TText *Act = new TText();
  //Act -> SetNDC();
  //Act -> SetTextFont(1);
  ...
  //Act -> DrawText(0.3, y*.7, Form("A(30Ne) = %f",p.first[3]));
  //TText *Act1 = new TText();
  //Act1 -> SetNDC();
  //Act1 -> SetTextFont(1);
  //Act1 -> SetTextColor(1);
  //Act1 -> SetTextSize(0.05);
  //Act1 -> DrawText(0.3, y*.65, Form("A(30Na) = %f",p.first[5]));
  //TText *Act2 = new TText();
  //Act2 -> SetNDC();
  //Act2 -> SetTextFont(1);
  //Act2 -> SetTextColor(1);
  //Act2 -> SetTextSize(0.05);
  //Act2 -> DrawText(0.3, y*.6, Form("A(29Na) = %f",p.first[7]));
  //TText *Act3 = new TText();
  //Act3 -> SetNDC();
  //Act3 -> SetTextFont(1);
  //Act3 -> SetTextColor(1);
  //Act3 -> SetTextSize(0.05);
  //Act3 -> DrawText(0.3, y*.55, Form("A(30Mg) = %f",p.first[9]));
...
void hlerrcheck(TH1D *hist, int rebin, double chi2_min=-1){
  ...
  for(int i=0;i<9;i++){
    binnum = (1000+500*i)/rebin;
    fx[i] = new TF1(Form("fx%i",i),MYFit,0,binnum*rebin/10,11);
    ...
    f1[i]->SetParameter(0,0);                                 // return parent curve
    ...
    f2[i]->SetParameter(2,p[i].first[3]);             // actvity
    f2[i]->SetParameter(3,p[i].first[4]);             // parent t1/2
    ...
    f3[i]->SetParameter(2,p[i].first[5]);             // acitivity
    f3[i]->SetParameter(3,p[i].first[4]);             // parent(30Ne) t1/2
    f3[i]->SetParameter(4,p[i].first[6]);             // daughter(30Na) t1/2
    ...
    f4[i]->SetParameter(2,p[i].first[7]);             // activity
    f4[i]->SetParameter(3,p[i].first[4]);             // parent(30Ne) t1/2
    ...
    f5[i]->SetParameter(2,p[i].first[9]);             // activity
    f5[i]->SetParameter(3,p[i].first[4]);             // parent(30Ne) t1/2
    f5[i]->SetParameter(4,p[i].first[6]);             // daughter(29Na) 1/2
    f5[i]->SetParameter(5,p[i].first[10]);             // grand(30Mg) t1/2
    ...
    chi2 -> SetTextColor(1);
    chi2 -> SetTextSize(0.05);
    chi2 -> DrawText(0.3, y*.85, Form("X2 = %.2f",p[i].first.back()));
    TText *hl = new TText();
    hl -> SetNDC();
    hl -> SetTextFont(1);
    hl -> SetTextColor(1);
    hl -> SetTextSize(0.05);
    if(p[i].second.size()<5 || p[i].second[4]<1e-6){
      hl -> DrawText(0.3, y*.8, Form("hl = %.2fms",p[i].first[4]));
    }else{
      hl -> DrawText(0.3, y*.8, Form("hl = %.2f(%.4f)ms",p[i].first[4], p[i].second[4]));
    } 
    TText *Act = new TText();
    Act -> SetNDC();
    Act -> SetTextFont(1);
    Act -> SetTextColor(1);
    Act -> SetTextSize(0.05);
    Act -> DrawText(0.3, y*.75, Form("A(30Ne) = %f",p[i].first[3]));
    TText *Act1 = new TText();
    Act1 -> SetNDC();
    Act1 -> SetTextFont(1);
    Act1 -> SetTextColor(1);
    Act1 -> SetTextSize(0.05);
    Act1 -> DrawText(0.3, y*.7, Form("A(30Ne->30Na) = %f",p[i].first[5]));
    TText *Act2 = new TText();
    Act2 -> SetNDC();
    Act2 -> SetTextFont(1);
    Act2 -> SetTextColor(1);
    Act2 -> SetTextSize(0.05);
    Act2 -> DrawText(0.3, y*.65, Form("A(30Ne->29Na) = %f",p[i].first[7]));
    TText *Act3 = new TText();
    Act3 -> SetNDC();
    Act3 -> SetTextFont(1);
    Act3 -> SetTextColor(1);
    Act3 -> SetTextSize(0.05);
    Act3 -> DrawText(0.3, y*.6, Form("A(30Ne->30Mg) = %f",p[i].first[9]));
...
/*void hlerrcheck(TH1D *hist){

  int size = 1;

  int rebin[size];
  int binnum;
  double chi2_min[size];
  TGraph *tempgr[size];
  std::pair<std::vector<double>, std::vector<double>> p[size];
  TH1D *th1[size]; 
  TF1 *fx[size];
  ...
  for(int i=0;i<size;i++){
    fx[i] = new TF1(Form("fx%i",i),MYFit,0,1000,10);
    fx[i]->SetLineColor(kRed);
    fx[i]->SetLineWidth(2);
    ...
    c[i] = new TCanvas;
    c[i]->SetTitle(Form("bin in %.1fms",rebin[i]/10.));
    c[i]->Divide(3,3);
    
    for(int j=0;j<9;j++){
      p[i].first.clear();
      p[i].second.clear();
      binnum = (1000+500*j)/rebin[i];
      th1[i] = (TH1D *)hist->Clone(Form("decaytime_%ims_rebin_%i",binnum/10,rebin[i]));
      th1[i]->Rebin(rebin[i]);
      tempgr[i] = convert_th1_tgr(th1[i], binnum);
      tempgr[i]->SetTitle(Form("decaytime = %ims, bin in %.1fms",binnum*rebin[i]/10,rebin[i]/10.));
      p[i] = searchinitPar(hist,rebin[i],binnum,chi2_min[i]);
      for(int npar=0;npar<p[i].second.size();npar++){
        fx[i]->SetParameter(npar,p[i].first[npar]);
      } 
      c[i]->cd(j+1);
      ...
      chi2 -> SetTextColor(1);
      chi2 -> SetTextSize(0.05);
      chi2 -> DrawText(0.5, y*7, Form("X2 = %.2f",p[i].first.back()));
      TText *hl = new TText();
      hl -> SetNDC();
      hl -> SetTextFont(1);
      hl -> SetTextColor(1);
      ...
      if(p[i].second.size()<5){
        hl -> DrawText(0.5, y*.65, Form("hl = %.2fms",p[i].first[4]));
      }else{
        hl -> DrawText(0.5, y*.6, Form("hl = %.2f(%.2f)ms",p[i].first[4], p[i].second[4]));
      }
      double A = p[i].first[3]+p[i].first[6]+p[i].first[7];
      TText *Act = new TText();
      Act -> SetNDC();
      Act -> SetTextFont(1);
      Act -> SetTextColor(1);
      Act -> SetTextSize(0.05);
      Act -> DrawText(0.5, y*.55, Form("A(30Ne) = %f",A));
      TText *Act1 = new TText();
      Act1 -> SetNDC();
      Act1 -> SetTextFont(1);
      Act1 -> SetTextColor(1);
      Act1 -> SetTextSize(0.05);
      Act1 -> DrawText(0.5, y*.5, Form("A(30Ne) = %f",A));
      TText *Act2 = new TText();
      Act2 -> SetNDC();
      Act2 -> SetTextFont(1);
      Act2 -> SetTextColor(1);
      Act2 -> SetTextSize(0.05);
      Act2 -> DrawText(0.5, y*.45, Form("A(30Ne) = %f",A));
      TText *Act3 = new TText();
      Act3 -> SetNDC();
      Act3 -> SetTextFont(1);
      Act3 -> SetTextColor(1);
      Act3 -> SetTextSize(0.05);
      Act3 -> DrawText(0.5, y*.4, Form("A(30Ne) = %f",A));
  ...
  TH1D *th1;
  TGraph *tempgr;
  TF1 *fx = new TF1("fx",MYFit,0,1000,10);
  std::pair<std::vector<double>, std::vector<double>> p;
  int rebin = 10; // bin in 0.5ms
  int chi2_min = 5;
  TCanvas *c1 = new TCanvas; 
  c1->SetTitle(Form("bin in %.1fms",rebin/10.));
  c1->Divide(3,3);
  for(int i=0;i<9;i++){  
    p.first.clear();
    p.second.clear();
    int binnum = (1000+500*i)/rebin;
    th1 = (TH1D *)hist->Clone(Form("decaytime_%ims",binnum/10));
    th1->Rebin(rebin);
    tempgr = convert_th1_tgr(th1, binnum);
    tempgr->SetTitle(Form("decaytime = %ims",binnum/10));
    p = searchinitPar(hist,rebin,binnum,chi2_min);
    for(int npar=0;npar<(p.first.size()-1);npar++){
      fx->SetParameter(npar,p.first.at(npar));
    }
    c1->cd(i+1);
    tempgr->Draw();
    fx->Draw("same");    
    ...
    //TLatex l1;
    //l1.SetTextSize(0.05);
    //l1.DrawLatexNDC(13,y*.5,Form("chi2 = %.4f",p.first.back()));
    //TLatex l2;
    //l2.SetTextSize(0.05);
    //l2.DrawLatexNDC(13,y*.4,Form("t_{1/2} = %.2f(%.2f)",p.first[4], p.second[4]));
    ...
    chi2 -> SetTextColor(1);
    chi2 -> SetTextSize(0.05);
    chi2 -> DrawText(0.5, y*.5, Form("X2 = %.2f",p.first.back()));
    TText *hl = new TText();
    hl -> SetNDC();
    hl -> SetTextFont(1);
    hl -> SetTextColor(1);
    hl -> SetTextSize(0.05);
    if(p.second.size()<5){
      hl -> DrawText(0.5, y*.4, Form("hl = %.2fms",p.first[4]));
    }else{
      hl -> DrawText(0.5, y*.4, Form("hl = %.2f(%.2f)ms",p.first[4], p.second[4]));
    }
    c1->Update(); 
  }
