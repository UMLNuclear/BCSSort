...
void Histogram::BetaSort(){
  ...
  for(x=0;x<entries;x++){
    ...
    for(int d=0;d<fbeta->DecaySize();d++){
      double decaytime = fbeta->fDecay[d].fDecayTime;
      if(Na32->IsInside(tof2,fimp.fPIN1E)){
        FillHistogram(Form("decaytime_%s",Na32->GetName()),30000,0,3000,decaytime);
      }
      if(NeCut->IsInside(tof2,fimp.fPIN1E)){
        FillHistogram(Form("decaytime_%s",NeCut->GetName()),30000,0,3000,decaytime);
      }
      if(NeCut0->IsInside(tof2,fimp.fPIN1E)){
        FillHistogram(Form("decaytime_%s",NeCut0->GetName()),30000,0,3000,decaytime);
      }
      ...
      for(int m=0;m<fbeta->fDecay[d].GeSize();m++){
        ...
        if(NeCut->IsInside(tof2, fimp.fPIN1E)){
          FillHistogram(Form("dt_singles_%s",NeCut->GetName()), 3000,0,3000,decaytime, 4000,0,4000,gamEm);
        }
        if(NeCut0->IsInside(tof2, fimp.fPIN1E)){
          FillHistogram(Form("dt_singles_%s",NeCut0->GetName()), 3000,0,3000,decaytime, 4000,0,4000,gamEm);
        }
      ...
      for(it1=clmap.begin();it1!=clmap.end();it1++){
        ...
        if(NeCut->IsInside(tof2, fimp.fPIN1E)){
          FillHistogram(Form("dt_addback_%s",NeCut->GetName()), 3000,0,3000,decaytime, 4000,0,4000,it1->second.AddbackE());
        }
        if(NeCut0->IsInside(tof2, fimp.fPIN1E)){
          FillHistogram(Form("dt_addback_%s",NeCut0->GetName()), 3000,0,3000,decaytime, 4000,0,4000,it1->second.AddbackE());
        }
