...
void Histogram::BetaSort(){
  ...
  runnum = runnum.substr(0,4);
  TChain *beta = new TChain("beta");
  beta->Add(Form("/home/zhu/packages/BCSSort/data/5us_tofcor/beta/beta_good_prompt_tofcor/beta%s*.root",runnum.c_str()));
  Beta *fbeta = new Beta;
  beta->SetBranchAddress("Beta",&fbeta);
  TChannel::ReadDetMapFile();
  ...
  for(int i=0;i<=16;i++){
    double startl[2] = {11800,8000};
    double startr[2] = {13000,8000};
    startl[1] += i*u;
    startr[1] += i*u;
    cutgl = new TCutG(Form("recl%i",i),5);
    cutgl->SetPoint(0,startl[0],startl[1]);
    cutgl->SetPoint(1,startl[0]-l,startl[1]);
    cutgl->SetPoint(2,startl[0]-l,startl[1]+w);
    cutgl->SetPoint(3,startl[0],startl[1]+w);
    cutgl->SetPoint(4,startl[0],startl[1]);
    gList->Add(cutgl);

    cutgr = new TCutG(Form("recr%i",i),5);
    cutgr->SetPoint(0,startr[0],startr[1]);
    cutgr->SetPoint(1,startr[0]-l,startr[1]);
    cutgr->SetPoint(2,startr[0]-l,startr[1]+w);
    cutgr->SetPoint(3,startr[0],startr[1]+w);
    cutgr->SetPoint(4,startr[0],startr[1]);
    gList->Add(cutgr);
  }
  ...
  for(x=0;x<n;x++){
    ...
    for(auto &it:fbeta->fDecay){
      if(Ne->IsInside(fimp.fI2S, fimp.fPIN1E)){ // Gate on Ne chain in PID (lock PIN1E)
        if((fimp.fI2S>11100 && fimp.fI2S<13000) && (fimp.fI2S_I2N>8000 && fimp.fI2S_I2N<16000)){ // New Gate on I2position and TOF
          if(flagimp){
            FillHistogram("PID_Ne31",250,11000,13500,fimp.fI2S, 200,4700,6700,fimp.fPIN1E);
            FillHistogram("I2_TOF_Ne31",190,11100,13000,fimp.fI2S,800,8000,16000,fimp.fI2S_I2N);
            flagimp = false;
            for(int mov=0;mov<=16;mov++){
              TCutG *recml = (TCutG *)gList->FindObject(Form("recl%i",mov));
              TCutG *recmr = (TCutG *)gList->FindObject(Form("recr%i",mov));
              if(recml->IsInside(fimp.fI2S, fimp.fI2S_I2N)){
                FillHistogram(Form("PID_recl%i",mov), 250,11000,13000,fimp.fI2S, 200,4700,6700,fimp.fPIN1E);
                FillHistogram(Form("I2_TOF_recl%i",mov),190,11100,13000,fimp.fI2S,800,8000,16000,fimp.fI2S_I2N);
                if(it.fDecayTime<=100){
                  FillHistogram(Form("decaysize_recl%i",mov), 100,0,100,fbeta->DecaySize());
                }
              }
              if(recmr->IsInside(fimp.fI2S, fimp.fI2S_I2N)){
                FillHistogram(Form("PID_recr%i",mov), 250,11000,13000,fimp.fI2S, 200,4700,6700,fimp.fPIN1E);
                FillHistogram(Form("I2_TOF_recr%i",mov),190,11100,13000,fimp.fI2S,800,8000,16000,fimp.fI2S_I2N);
                if(it.fDecayTime<=100){
                  FillHistogram(Form("decaysize_recr%i",mov), 100,0,100,fbeta->DecaySize());
                }
              }
            }
          }
          ...
          for(auto &it2:it.fGe){
            ...
            clmap[clnum].Add(it2);
            FillHistogram("decaytime_single",2000,0,200,it.fDecayTime,4000,0,4000,it2.GetEnergy());
            FillHistogram("decaytime_single_200",2000,200,400,it.fDecayTime,4000,0,4000,it2.GetEnergy());
            ...
            for(int mov=0;mov<=16;mov++){
              ...
              TCutG *recmr = (TCutG *)gList->FindObject(Form("recr%i",mov));
              if(recml->IsInside(fimp.fI2S,fimp.fI2S_I2N)){
                FillHistogram(Form("dt_single_recl%i",mov), 1000,0,100,it.fDecayTime, 4000,0,4000,it2.GetEnergy());
              }
              if(recmr->IsInside(fimp.fI2S,fimp.fI2S_I2N)){
                FillHistogram(Form("dt_single_recr%i",mov), 1000,0,100,it.fDecayTime, 4000,0,4000,it2.GetEnergy());
              }
            }
            //ggmatrix
            for(auto &it3:it.fGe){
              ...
              FillHistogram("ggmat70", 4000,0,4000,it2.GetEnergy(), 4000,0,4000,it3.GetEnergy());
              if(it.fDecayTime>50) continue;
              FillHistogram("ggmat50", 4000,0,4000,it2.GetEnergy(), 4000,0,4000,it3.GetEnergy());
              if(it.fDecayTime>30) continue;
              FillHistogram("ggmat30", 4000,0,4000,it2.GetEnergy(), 4000,0,4000,it3.GetEnergy());
              if(it.fDecayTime>15) continue;
              FillHistogram("ggmat15", 4000,0,4000,it2.GetEnergy(), 4000,0,4000,it3.GetEnergy());
              if(it.fDecayTime>10) continue;
              FillHistogram("ggmat10", 4000,0,4000,it2.GetEnergy(), 4000,0,4000,it3.GetEnergy());
            }
          }
          //============ Addback ==============//
          ...
          for(it4=clmap.begin();it4!=clmap.end();it4++){
            it4->second.SetAddE();
            if(it.fDecayTime>100) continue;
            FillHistogram("decaytime_ab",1000,0,100,it.fDecayTime,4000,0,4000,it4->second.AddbackE());
            for(int mov=0;mov<11;mov++){
              TCutG *recml = (TCutG *)gList->FindObject(Form("recl%i",mov));
              TCutG *recmr = (TCutG *)gList->FindObject(Form("recr%i",mov));
              if(recml->IsInside(fimp.fI2S,fimp.fI2S_I2N)){
                FillHistogram(Form("dt_ab_recl%i",mov), 1000,0,100,it.fDecayTime, 4000,0,4000,it4->second.AddbackE());
              }
              if(recmr->IsInside(fimp.fI2S,fimp.fI2S_I2N)){
                FillHistogram(Form("dt_ab_recr%i",mov), 1000,0,100,it.fDecayTime, 4000,0,4000,it4->second.AddbackE());
              }
            }
          }
        }
