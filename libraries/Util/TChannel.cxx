...
TChannel* TChannel::FindChannel(int number)   {
  TChannel *chan = 0;
  if(number<0)
     return chan;
  for(auto &iter : fChannelMap) {
     chan = iter.second;
     if(number==chan->GetNumber())
        break;
     chan = 0;
  }
  return chan;
...
bool TChannel::AddChannel(TChannel* chan,Option_t *opt) {
  ...
  if(fChannelMap.count(chan->GetAddress())==1) {
     if(option.Contains("overwrite",TString::kIgnoreCase)) {
       ...
       fprintf(stderr,"%s: Trying to add a channel that already exists! [0x%08x]  \n",__PRETTY_FUNCTION__,chan->GetAddress());
       return false;
     }
...
/*void TChannel::SetCrosstalkCoeff(std::vector<double> coeff) {
  for(size_t i=0;i<coeff.size();i++){
    crosstalk_coeff.push_back(coeff.at[i]);
  }
}*/
...
/*double TChannel::FixCrosstalk(const std::vector<double>& energy, const std::vector<double>& coeff){
  double newe = 0;
  for(size_t x=0;x<energy.size();x++){
    newe += energy.at[x]*coeff.at[x];
  }
  return newe;
}*/
...
int TChannel::ReadDetMapFile(const char* filename,Option_t *opt) {
  std::string infilename = filename;
  if(infilename.length()==0)
     return -1;
  ...
  printf("Read  %i channels from %s \n", ChannelFound, infilename.c_str());
